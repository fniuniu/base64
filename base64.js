//base64解码


 //char 是一个长度为 1 的 string
 //这个函数返回这个字符的 ASCII 码

var ascii = function(char) {
   
    return char.charCodeAt(0)
}


/*
    code 是一个 int
    返回 code 所表示的字符

    这个答案用到了 String.fromCharCode(code) 函数, 例子如下
    String.fromCharCode(97) 返回 'a'
    String.fromCharCode(65) 返回 'A'
*/
var charFromAscii = function(code) {
    return String.fromCharCode(code)
}


/*
    n 是一个不大于 255 的 int
    返回 n 的 8 位二进制形式的字符串
    例如 binary(7) 返回 '00000111'
*/
var binary = function(n) {
    
    var l = ''
    while(n != 0) {
        var a = n % 2
        n = Math.floor(n / 2)
        l = a + l
    }
    for (var i = 0; l.length < 8; i++) {
        l = "0" + l
    }
    return l
}

/*
    bin 是一个 8 位二进制形式的字符串
    返回 bin 代表的数字
    例如 int('00000111') 返回 7

    进制转换自行搜索或者论坛提问大家讨论吧
    */
var int = function(bin) {
    
    var len = bin.length
    var l = 0
    for (var i = 0; i < len; i++) {
        var e = bin[i]
        l = l + e * Math.pow(2, len-i-1)
    }
    return l
}


//
var binaryStream = function(s) {
    /*
    s 是一个 string
    返回 s 的二进制字符串
    例如 binaryStream('Man') 返回
    '010011010110000101101110'

    使用上面的函数
    */
    var n = ''
    for (var i = 0; i < s.length; i++) {
        var e = s[i]
        n = n + binary(ascii(e))
    }
    return n
}


//
var stringFromBinary = function(bins) {
    /*
    bins 是一个二进制形式的字符串
    返回 bins 代表的原始字符串
    例如 stringFromBinary('010011010110000101101110') 返回 'Man'

    使用上面的函数
    */
    var l = ''
    for (var i = 0; i < bins.length; i=i+8) {
        var n = bins.slice(i, i+8)
        l = l + charFromAscii(int(n))
    }
    return l
}

// 作业 7
//
var charFromBase64 = function(code, or) {
    var words = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
    var e = words[code]
    if(or) {
       e = e.replace('A', '=')
    }
    return e
}

var stringFromBase64 = function(bins, or) {
    var l = ''
    var len = bins.length
    for (var i = 0; i < len; i=i+6) {
        var n = bins.slice(i, i+6)
        l = l + charFromBase64(int(n),or)
    }
    return l
}

var base64Encode = function(s) {
    /*
    s 是一个 string
    返回 s 的 base64 编码

    Base64是一种基于 64 个可打印字符来表示数据的方法
    它用每6个比特为一个单元，对应某个可打印字符
    ASCII 码一个字符是 8 比特, 也就是一字节
    3 个字节就有 24 个比特, 对应了 4 个 base64 单元

    如下所示
    原始信息        M        a        n
    ASCII         77       7        110
    二进制         01001101 01100001 01101110
    4 个单元       010011 010110 000101 101110
    每个单元转换后  19  22  5  46

    转换后每个 base64 单元都是一个 0-63 的数字
    因为 6 比特表示的范围就是这么大
    然后数字到字符串的转换是下面这段字符串取下标所得
    'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'

    那么 Base64 编码结果就是    T   W   F  u

    所以 base64Encode('Man') 返回 'TWFu'


    既然 3 个字节转换为 4 个 base64 单元
    那么 1 个字节怎么办呢?
    答案是用 0 补出 3 字节, 如下所示
    原始信息    M
    ASCII     77       0        0
    二进制     01001101 00000000 00000000
    4 个单元   010011 010000 000000 000000
    单元转换后  19 16 0 0
    因为末尾是强行补上的, 所以给他用 '=' 凑出字符(这是一个例外)
    所以 base64Encode('M') 返回 'TQ=='
    */
    var n = binaryStream(s)
    var or = 0
    while(n.length%6 != 0) {
        n = n + '00000000'
        or++
    }
    return stringFromBase64(n, or)
}

// 作业 8
//
var Base64 = function(char) {
    var words = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/'
    for(var i = 0; i < words.length; i++) {
        if(char === words[i]) {
            return i
        }
    }
    return 0
}

var binaryBase64 = function(n) {
    var l = ''
    while(n != 0) {
        var a = n % 2
        n = Math.floor(n / 2)
        l = a + l
    }
    for (var i = 0; l.length < 6; i++) {
        l = "0" + l
    }
    return l
}

var binaryStreamBase64 = function(s) {
    var n = ''
    for (var i = 0; i < s.length; i++) {
        var e = s[i]
        var or = 0
        if('=' === e) {
            or++
        }
        n = n + binaryBase64(Base64(e))
        var len = n.length
        n = n.substring(0, len - 8*or)         //删掉（or*8）个0
    }
    return n                                       //8位 二进制
  }

var base64Decode = function(s) {
    /*
    s 是一个 base64 编码后的字符串
    解码 s 并返回
    例如 base64Decode('TWFu') 返回 'Man'
    */
    return stringFromBinary(binaryStreamBase64(s))
}
base64Decode('TWFu')
